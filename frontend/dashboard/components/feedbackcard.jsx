...
export default function FeedbackCard({ eventId }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const meRes = await fetch(`${API}/events/${eventId}/ratings/me`, { credentials: 'include' });
        if (meRes.status === 200) {
        ...
        const sumRes = await fetch(`${API}/events/${eventId}/ratings/summary`, { credentials: 'include' });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Refresh summary
      const s = await fetch(`${API}/events/${eventId}/ratings/summary`, { credentials: 'include' }).then(r => r.json());
      setSummary(s);
    } catch (e) {
      setError(e.message);
    } finally {
          ...
          {isSubmitted ? (
              ...
              <>
                ...
                <div className="space-y-3">
                  <label className="block text-sm font-semibold text-gray-700">Rate your experience</label>
                  <div className="flex justify-center space-x-2">
                    {[1,2,3,4,5].map(star => (
                        <button key={star} onClick={() => setRating(star)} className="focus:outline-none transform hover:scale-110 transition-all duration-200">
                          <Star
                              className={`w-8 h-8 ${star <= rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300 hover:text-yellow-300'} transition-colors duration-200`}
                          />
                        </button>
                    ))}
                  </div>
                  {mine && <p className="text-xs text-gray-500 text-center bg-blue-50 rounded-lg py-2 px-3">Previous rating: {mine.rating} ⭐</p>}
                </div>

                {/* Comment */}
                <div className="space-y-3">
                  <label className="block text-sm font-semibold text-gray-700">Share your thoughts</label>
                  <div className="relative">
                <textarea
                    value={feedback}
                ...
                />
                    <div className="absolute bottom-3 right-3 text-xs text-gray-400">{feedback.length}/500</div>
                  </div>
                </div>

                {error && <div className="bg-red-50 border border-red-200 rounded-xl p-4"><p className="text-red-600 text-sm font-medium">{error}</p></div>}
                ...
                >
                  <div className="flex items-center justify-center space-x-3">
                    ...
                    <span className="text-lg">{isSubmitting ? 'Sending...' : 'Submit Feedback'}</span>
                  </div>
                ...
                <p className="text-xs text-gray-500 text-center italic">Your feedback makes our events better ✨</p>
              </>
          )}
        </div>
      </div>
